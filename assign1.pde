...
void draw() {
  ...
  if (button) {
    ...
    if (rolling){
      ...
      int da=int(random(6));
      machine.setSlotFruit(0,da);
         
      
      int db=int(random(6));  
      machine.setSlotFruit(1,db);
      ...
      totalScore+=
      machine.getSlotScore(da)*machine.getFruitCount(da)+
      machine.getSlotScore(db)*machine.getFruitCount(db)+
      machine.getSlotScore(dc)*machine.getFruitCount(dc);
